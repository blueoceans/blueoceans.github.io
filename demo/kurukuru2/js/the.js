...
function maskJPEG(val, useImageMask) {
	...
	function ByteString( string) {
	    ...
	    function nth(n) {
		if ( typeof data == "string") return string.charCodeAt(n) & 0xff;
		else return IEBinary_getByteAt(string,n);  // I have no IE but this looks promising.
	    };
	    function next() { return nth(p++); }
	    function skip(n) { p += n; };
	    function nextBytes(n) { 
		if ( n == undefined) n = string.length - p;
		...
		var r = '';
		for ( var i = p; i < p+n; i++) r = r + String.fromCharCode( nth(i));
		p += n;
		return new ByteString(r);
	    };
	    function backup() { if ( --p < 0) p = 0; };
	    function toString() { return string.substring(0); }
	...
	function JPEG( bytes) {
		...
		for (;;) {
		    ...
		    if ( a != 0xff) continue;
		    var b = bytes.next();
		    if ( b == 0) continue; // an escaped 0xff
		...
		// Tags without values
		if ( tag >= 0xd0 && tag <= 0xd9 || tag == 0) return { tag: tag, length: 0, value:undefined };
	...
	if ( contentType && contentData) {
		...
		val.setAttribute('data-alpha-src',url);
...
$(document).ready( function() {	
	...
	$('IMG.please-mask-with-canvas').each( function(idx,val) { maskJPEG(val); }); 
	$('IMG.please-mask-with-css').each( function(idx,val) { maskJPEG(val, true); }); 
    });